...
#### 4.<span id="stl">STL</span>
- 六大组件 
- STL缺点
- vector，list的特点和使用场景，底层实现，区别
- vector的扩容
- 推导往一个 vector 插入N个元素的时间复杂度
- vector如何释放内存

#### 5.<span id="c11">C++11</span>
- auto
- override和final
- 括号初始化
...
### 多态的原理：
1.在含有虚函数的类中，在对象的前面或后面存放了一个虚函数指针，这个指针存在虚函数表中，虚表存在代码段。
智能指针的RAII特性和好处：
利用对象的生命周期来控制程序资源（如内存、网络连接），构造时获取资源，析构时释放，将管理一份资源的责任托付给了一个对象。
好处：
不需要显式释放资源
对象所需资源在对象生命周期始终有效

### auto_ptr的问题：
当对象拷贝或者赋值之后，前面的对象就悬空了。
...
不需要显式释放资源
对象所需资源在对象生命周期始终有效
C++不像java有垃圾回收机制，智能指针解决了内存泄漏的部分问题
2.智能指针还要重载operator*和operator->，让智能指针具有指针的行为

### auto_ptr的实现和缺陷：
ap本质是转移管理权，当对象拷贝或者赋值之后，ap会把管理资源转移到当前对象中，自身ap=NULL也就是将指针赋空不再管理这份资源。
缺陷：转移会造成悬空，当对象拷贝或者赋值之后，ap会把管理资源转移到当前对象中，自身ap=NULL也就是将指针赋空不再管理这份资源，给自己赋空为了防止一份资源被多个对象使用，但是此时ap对象就悬空了，通过ap访问资源就会出问题。

### unique_ptr的原理和实现:
既然ap拷贝和赋值会出问题，那我up直接防拷贝保证了安全。
如何防拷贝：C++11的delete函数，在函数声明后面加上 =delete 编译器会直接禁用此函数。

### shared_ptr:
使用引用计数的方式，实现多个shared_ptr共享资源，引用计数记录当前资源被几个智所共享。
底层：
1.对象被销毁时，调用析构函数，说明当前对象不需要资源了，引用计数-1。
2.引用计数为0时，说明自己是最后一个使用此资源的对象，必须释放资源。
3.不是0就说明还有别的对象在使用资源，不能释放，否则别的对象就变成野指针了。

### shared_ptr的线程安全问题：
引用技术的线程安全：引用计数是共享的，如果++和--操作被多个智能指针同时操作，这个++--不是原子的，可能++了2次，结果发现引用计数只++了1
如何解决：对++和--操作加锁，保证了线程安全

### shared_ptr的循环引用问题：
例子：
1.AB两个shared_ptr对象指向两个节点CD，引用计数为1
...
### vector，list的特点和使用场景，底层实现，区别：
  vector：连续存储 list：不连续存储
  1、如果你需要高效的随即存取，而不在乎插入和删除的效率，使用vector
  2、如果你需要大量的插入和删除，而不关心随即存取，则应使用list

### vector的扩容：
  1.元素插入导致的扩容：
    初始化的时候，会额外分配一些空间应对可能的增长,size < capacity.
    当插入元素，size = capacity，也就是capacity满了的时候，capacity会扩容1.5倍(vs)或者2倍(g++)，
    倍数跟系统无关，跟系统使用哪个STL版本有关
  2.resize和reverse导致的扩容：
    reverse扩大预留空间，但是不改变容器内元素个数。也就是capacity扩大，但size不变。
    resize扩大预留空间，并且增加元素个数并且初始化。也就是capacity扩大，size也扩大。

### 往一个 vector 插入N个元素的时间复杂度：
    O(N)，扩充N次，就要扩充logN次空间，把拷贝次数加起来，就是0+1+4+8...2^logN=2N次，所以是O(N)

### vector如何释放内存：
  clear()只能清空对象，但是不能释放内存。
  vector.swap()，vector会调用默认构造生成临时vector，然后临时vector调用swap，
  函数结束之后空间被交换，原来的空间变成临时vector的空间，然后随着析构函数一起被清理。
  而vector的空间变成默认构造函数的默认空间，也就完成了清理
  （swap交换技巧实现内存释放思想：vector()使用vector的默认构造函数建立临时vector对象，
  再在该临时对象上调用swap成员，swap调用之后对象myvector占用的空间就等于一个默认构造的对象的大小，
  临时对象就具有原来对象v的大小，而该临时对象随即就会被析构，从而其占用的空间也被释放。）
  
### auto关键字：
基于范围的for循环，冒号前面是迭代的变量i，后面是一个范围。定义auto变量，需要有初始化表达式，编译器通过初始化表达式推导标量类型。
### override和final修饰虚函数：
  override：强制要求你重写，如果不重写，编译器就会报错
  final：禁止你重写，如果重写，就会报错
